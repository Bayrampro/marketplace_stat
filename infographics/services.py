...
def _build_openai_prompt(title, keywords):
    ...
    return (
        "Задача: добавить поверх загруженной фотографии только дизайн-слой "
        "инфографики. Исходное фото должно остаться тем же самым.\n\n"
        "Строгие запреты: не генерируй новый товар, новую модель, новую одежду "
        "или новую сцену; не меняй лицо, позу, тело, руки, ноги, ткань, цвет, "
        "форму, фактуру, освещение, фон, ракурс и композицию исходного изображения; "
        "не ретушируй и не дорисовывай предметы. Не закрывай главный товар крупными "
        "плашками. Допустимо только наложить графику и текст поверх фото.\n\n"
        "Что нужно добавить: красивую маркетплейс-инфографику для карточки "
        "Wildberries/Ozon: крупный читаемый заголовок, 2-4 аккуратные плашки, "
        "короткие подписи, тонкие линии/маркеры или маленькие иконки. Все слова "
        "должны быть на русском и взяты из данных ниже.\n\n"
        f"Заголовок: {title.strip()}\n"
        f"Ключевые слова/фразы для плашек: {keyword_text}\n\n"
        "Визуальный стиль: современная карточка товара, чисто, контрастно, "
        "профессионально, без лишнего декора. Не используй подписи "
        "«Преимущество 1», «Преимущество 2», «Преимущество 3». Итог должен "
        "выглядеть как исходное фото с наложенной инфографикой, а не как "
        "пересозданная фотография."
    )
